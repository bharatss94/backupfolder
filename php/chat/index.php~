...
if ($_SESSION['member_id'] && $_SESSION['member_status'] == 'active' && $_SESSION['member_role']) {
    if ($_GET['action'] == 'update_last_nav') { // update last navigate time
        ...
        if ($iPid) {
           // $GLOBALS['MySQL']->res("UPDATE `cs_profiles` SET `active` = '1' WHERE `id` = '{$iPid}'");
...
{
    echo $GLOBALS['CProfiles']->getProfilesBlock(10, true);
}*/
...
//updating online
/*if ($_GET['action'] == 'getOnline') { // regular updating of messages in chat
        $sProfiles = $GLOBALS['CProfiles']->getProfilesBlock();
$sOnlineMembers = $GLOBALS['CProfiles']->getProfilesBlock(10, true);
...
//echo strtr(file_get_contents('templates/online.html'), $aKeys);
/*  looping continously
while (true) {
$sProfiles = $GLOBALS['CProfiles']->getProfilesBlock();
$sOnlineMembers = $GLOBALS['CProfiles']->getProfilesBlock(10, true);
...
//trying to echo the main page html every sec using set interval.
//setInterval(echo strtr(file_get_contents('templates/main_page.html'), $aKeys),1000);//1 sec
/**function to refresh 
